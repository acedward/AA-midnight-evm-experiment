...
now require `wit_OwnableSK`, mint entry points reject non-owners,
`burn(value)` is replaced by `_burn(account,value)`, `transferWithEthSig` is
replaced by `accountTransfer(Account,Bytes<176>,sig,pk)`, and
`mintToEthAddress` is replaced by `mintToAccountAddress`. Existing clients and
proofs must regenerate against TokenAA's artifact.
